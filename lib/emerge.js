...
function immutableClone (value) {
  return isPrimitive(value)
    ...
    : mapValuesDroppingNil(value, immutableClone)
}
...
function nestedValue (value, path) {
  return path.slice().reverse().reduce(enclose, value)
}
...
function replaceObject (prev, next) {
  return mapRightPairsDroppingNil(prev, next, replace)
}

function mergeObject (prev, next) {
  return mergePatch(prev, mapRightPairsDroppingEqual(prev, next, merge))
}

function mergePatch (object, patch) {
  return size(patch)
    ? extendObjectDroppingNil(object, patch)
    : object
}
...
function arraysDeepEqual (one, other) {
  return one.length === other.length && everyArrayPair(one, other, deepEqual)
}
...
function mapValuesDroppingNil (object, func) {
  const buffer = {}
...
function mapRightPairsDroppingNil (left, right, func) {
  const buffer = {}
...
function mapRightPairsDroppingEqual (left, right, func) {
  const buffer = {}
  for (const key in right) {
    const value = func(left[key], right[key])
    if (!deepEqual(left[key], value)) buffer[key] = value
  }
  return freeze(buffer)
}

function extendObjectDroppingNil (object, patch) {
  const buffer = {}
  for (const key in object) {
    if (!(key in patch) || patch[key] != null) buffer[key] = object[key]
  }
  for (const key in patch) {
    if (patch[key] != null) buffer[key] = patch[key]
  }
  return freeze(buffer)
}
