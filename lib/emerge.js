...
export function readAtPath (tree, path) {
...
export function replaceAtRoot (prev, next) {
...
export function mergeAtRoot (prev, next) {
  const value = replaceIfNotPatchable(prev, next)
  if (value !== secret) return value

  const partial = {}
  const keysToRemove = Object.keys(next).filter(key => { return next[key] === undefined })
  ...
  const keysNext = Object.keys(partial)
  const keysToCopyPrev = arrayDiff(keysPrev, keysToRemove)
  const keysToCopyNext = arrayDiff(keysNext, keysToRemove)

  if (keys.length) {
    const buffer = {}
    keysToCopyPrev.forEach(key => {buffer[key] = prev[key]})
    keysToCopyNext.forEach(key => {buffer[key] = partial[key]})
    return Object.freeze(buffer)
  }
...
export function replaceAtPath (prev, next, path) {
  if (!(path instanceof Array)) throw TypeError(`path must be an array, got: ${path}`)
...
/*
  Substract sub2 array from sub1, returns new array.
  arrayDiff([1, 2, 3, 4], [2, 3])
    => [1, 4]
*/
function arrayDiff(sub1, sub2) {
  const a = new Set(sub1)
  const b = new Set(sub2)
