...
function readAtPath (tree, path) {
  ...
  for (let i = -1; ++i < path.length;) {
    if (isPrimitive(step)) return undefined
    const key = path[i]
    step = step[key]
  }
...
// Makes an immutable deep clone, ignoring keys with undefined values.
function cloneDeep (value) {
  ...
  each(Object.keys(value), key => {
    if (value[key] !== undefined) buffer[key] = cloneDeep(value[key])
  })
  return Object.freeze(buffer)
}
...
function immute (value) {
  if (isPrimitive(value) || Object.isFrozen(value)) return value
  // Ignore non-array properties in arrays.
  if (value instanceof Array) each(value, immute)
  else each(Object.keys(value), key => { immute(value[key]) })
